...
class DPGMM:
  ...
  def __init__(self, dims, stickCap = 1):
    ...
    if isinstance(dims, DPGMM):
      ...
      self.n = [GaussianPrior(x) for x in range(dims.n)]# map(lambda x: GaussianPrior(x), dims.n)
      ...
      self.nT = [None if x==None else StudentT(x) for x in dims.nT]# map(lambda x: None if x==None else StudentT(x), dims.nT)
      ...
      self.priorT = None
      self.n = [GaussianPrior(self.dims) for _ in range(self.stickCap)] #map(lambda _: GaussianPrior(self.dims), range(self.stickCap)) # The mixture components, one for each stick.
  ...
  def sampleMixture(self):
    ...
    gauss = [  x.sample() for x in self.n]#map(lambda x: x.sample(), self.n)
