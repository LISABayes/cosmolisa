...
if __name__=="__main__":
    ...
    axs = spec.subplots(sharex=False, sharey=False)
    ...
    cbar = fig.colorbar(C, location='top', ax=axs[0], aspect=40, pad=0.01)
    ...
    # cbar2 = fig.colorbar(matplotlib.cm.ScalarMappable(norm=norm, cmap=cmap),  location='right', ax=axs[0], aspect=20, pad=0.0)
    ...
    axs[0].set_xlim(np.min(redshift)*0.95,9)
    axs[0].set_ylim(0.0,90)
    ...
    # for ax in axs:
    #     ax.label_outer()
    fig.savefig(os.path.join(opts.outdir,'regression_MBHB_test.pdf'),bbox_inches='tight')
