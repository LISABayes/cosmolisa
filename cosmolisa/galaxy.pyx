...
from libc.math cimport pow, log, log10, isfinite, exp, sqrt, cos, \
    fabs, sin, sinh, M_PI, erf, erfc, HUGE_VAL, log1p
cimport cython
...
cdef class GalaxyDistribution:
    """Returns a Galaxy distribution function in magnitude
    and redshift. To obtain a pure Schechter function,
    set zmin = zmax = 0.0 and always evaluate the
    distribution at z = 0.
    ...
    cdef double _evaluate(self, double log10M, double z) nogil:
        ...
        return (ln10*phistar * exp(-p10logdiff) * p10logdiff**(-0.4*(As+1))
    ...
    cdef double _evaluate_detected(self, double log10M, double z) nogil:
        ...
        if (log10M > _absolute_magnitude(self.logMthreshold, 
                                         self.omega._LuminosityDistance(z))):
            return 0
        else:
            return (ln10*phistar * exp(-p10logdiff) 
                    * p10logdiff**(-0.4*(As+1))
                    * self.omega._ComovingVolumeElement(z))
    ...
    @cython.nonecheck(False)
    @cython.cdivision(True)
    cdef double _get_number_of_galaxies(self, double zmin, double zmax, 
                                        int selection) nogil:
        ...
        for i in range(N):            
            ...
            for j in range(N):
                if (selection == 0):
                    result += self._evaluate(m, z)
                elif (selection == 1):
                    result += self._evaluate_detected(m, z)
                elif (selection == 2):
                    result += self._evaluate_non_detected(m, z)
                m += dm
            z += dz
    ...
    @cython.nonecheck(False)
    @cython.cdivision(True)
    cdef double _get_detected_normalisation(self, double zmin, 
                                            double zmax) nogil:
        if (self._detected_normalisation == -1):
            self._detected_normalisation = self._get_number_of_galaxies(
                    zmin, zmax, 1)
    ...
    @cython.nonecheck(False)
    @cython.cdivision(True)
    cdef double _get_non_detected_normalisation(self, double zmin,
                                                double zmax) nogil:
        if (self._non_detected_normalisation == -1):
            self._non_detected_normalisation = self._get_number_of_galaxies(
                    zmin, zmax, 2)
    ...
    cdef double _pdf(self, double m, double z) nogil:
        return self._evaluate(m, z)/self._get_normalisation(self.zmin,
                                                            self.zmax)
    ...
    cdef double _pdf_detected(self, double m, double z) nogil:
        return (self._evaluate_detected(m, z)
                / self._get_detected_normalisation(self.zmin, self.zmax))
    ...
    cdef double _pdf_non_detected(self, double m, double z) nogil:
        return (self._evaluate_non_detected(m, z)
                / self._get_non_detected_normalisation(self.zmin, self.zmax))
    ...
    def sample_correlated(self, int N, double zmin, double zmax,
                          double ramin, double ramax, double decmin,
                          double decmax, int selection=0):
        return self._sample_correlated(N, zmin, zmax, ramin, ramax,
                                       decmin, decmax, selection=selection)
    
    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.nonecheck(False)
    @cython.cdivision(True)
    cdef np.ndarray[double, mode='c', ndim=2] _sample_correlated(
                self, int N, double zmin, double zmax, double ramin,
                double ramax, double decmin, double decmax, int selection):
        """Wwe want to sample a set of galaxies following the
        empirically measured correlation function as well as
        the global distribution of galaxies.
        
        We start by noting that given a galaxy, the probability
        of finding another one within a volume dV is given by
        
        dP = n(1+eps(r))dV = n(1+eps(r))(dV/dz) dz
        
        If we enlarge the discourse to include the magnitude,
        we can write
        ...
        cdef np.ndarray[double, mode='c', ndim=1] comoving_distance = (
        ...
        cdef np.ndarray[double, mode='c', ndim=2] out = np.zeros(
                (N,5), dtype=np.double)
        cdef double M, Z, RA, DEC
        cdef double D, corr = 0
        # Begin by sampling a redshift, a magnitude and a sky position
        # within the box. Note that we are going to ignore correlations
        # in the luminosities that are there in the universe.
        cdef np.ndarray[double, mode='c', ndim=1] r = np.linspace(
                0.0, 1000, 1000)
    ...
    @cython.nonecheck(False)
    @cython.cdivision(True)
    cdef tuple _sample(self, double zmin, double zmax, double ramin,
                       double ramax, double decmin, double decmax,
                       int selection):
    ...
    def sample(self, double zmin, double zmax, double ramin, double ramax,
               double decmin, double decmax, int N, int selection=0):
        return np.array([self._sample(zmin, zmax, ramin, ramax, decmin,
                        decmax, selection=selection) for _ in range(N)])
    ...
    @cython.nonecheck(False)
    @cython.cdivision(True)
    cdef double _loglikelihood(self, const double[::1] M,
                               const double[::1] Z) nogil:
        ...
        cdef double logL = (gammaln(Ntot+1) - gammaln(Ndet+1) 
                            - gammaln(Ntot-Ndet+1))
        for i in range(Ndet):
            logL += log(self._pdf(M[i], Z[i]))
        if (Ntot - Ndet) > 0.0:
            # See Eq.C8 in https://iopscience.iop.org/article/10.1088/0004-637X/786/1/57/pdf.
            logL += ((Ntot - Ndet) * log1p(-self._get_detected_normalisation(
                     self.zmin, self.zmax)/Ntot))
        return logL
    ...
    @cython.nonecheck(False)
    @cython.cdivision(True)
    cdef double _luminosity_function(self, double M, double Z,
                                     int selection) nogil:
...
cdef class SchechterFunctionMagnitude:
    """Returns a Schechter magnitude function for a given set
    of parameters.
    ...
    def __cinit__(self, double hubble_parameter, double Mstar, double alpha,
                  double mmin, double mmax, double phistar=1.):
...
cdef class GalaxyDistributionLog:
    """Returns a Comoving Volume element weighted
    Schechter magnitude function for a given set of parameters.
    p(z,  M|\Omega) dV/dz dz d\alpha d\delta dM. 
    Allows for evolving Mstar and alpha
    ...
    def __cinit__(self,
        ...
        self.slope_model_choice = slope_model_choice
        self.cutoff_model_choice = cutoff_model_choice
        self.alpha = slope_model(self.alpha0, self.alpha_exponent, 
                                 self.slope_model_choice)
        self.Mstar = cutoff_model(self.Mstar0, self.Mstar_exponent,
    ...
    cdef double _evaluate_detected(self, double m, double z):
        cdef double Mstar = self.Mstar(z)
        cdef double Mth = _absolute_magnitude(
                self.apparent_magnitude_threshold,
                self.omega._LuminosityDistance(z))
        if (m < Mth):
            return p4log10 \
                    * pow(10.0, -0.4*(self.alpha(z)+1.0)*(m-Mstar)) \
    ...
    cdef double _pdf_detected(self, double m, double z):
        return (self._evaluate_detected(m, z)
                / self._get_detected_normalisation())
    ...
    cdef double _pdf_non_detected(self, double m, double z):
        return (self._evaluate_non_detected(m, z)
                / self._get_non_detected_normalisation())
...
#        return np.array([self._sample() for _ in range(N)])
    ...
    cdef double _loglikelihood(self, double[:,::1] data):
        ...
        cdef double logL = (_log_stirling_approx(Ntot)
                            - _log_stirling_approx(Ndet)
                            - _log_stirling_approx(Ntot-Ndet))
...
cdef class slope_model:
    ...
    def __cinit__(self, double alpha0, double evolution, int model_choice):
        ...
        if (self.model_choice == 0):
            ...
            print(f"Model {self.model_choice} not supported!")
            exit()
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef inline double _phistar_evolution(double theta, double gamma,
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef inline double _absolute_magnitude(double apparent_magnitude,
                                       double dl) nogil:
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef inline double _apparent_magnitude(double absolute_magnitude,
                                       double dl) nogil:
...
cdef double objective(double z, CosmologicalParameters omega,
                      double dc) nogil:
...
cdef double _distance_spherical(
        np.ndarray[double, mode='c', ndim=1] x1, 
        np.ndarray[double, mode='c', ndim=1] x2):
    cdef double d1 = x1[0]**2 + x2[0]**2
    cdef double d2 = (-2.0*(x1[0]*x2[0]) * cos(x1[1]-x2[1]) -2.0*(x1[0]*x2[0])
                      * sin(x1[1]) * sin(x2[1]) * (cos(x1[2]-x2[2])-1.0))
    return sqrt(d1+d2)
